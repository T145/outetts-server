...
def chn2num(chinese_string, numbering_type=NUMBERING_TYPES[1]):
    ...
    def correct_symbols(integer_symbols, system):
        ...
        if integer_symbols and isinstance(integer_symbols[0], CNU) and integer_symbols[0].power == 1:
            integer_symbols = [system.digits[1]] + integer_symbols

        if len(integer_symbols) > 1 and isinstance(integer_symbols[-1], CND) and isinstance(integer_symbols[-2], CNU):
...
def num2chn(
    ...
    result_symbols = get_value(int_string) if use_units and len(int_string) > 1 else [system.digits[int(c)] for c in int_string]
    ...
    if alt_two:
        ...
        for i, v in enumerate(result_symbols):
            if isinstance(v, CND) and v.value == 2:
                ...
                if isinstance(next_symbol, CNU) and isinstance(previous_symbol, (CNU, type(None))) and next_symbol.power != 1 and ((previous_symbol is None) or (previous_symbol.power != 1)):
...
def remove_space(text):
    ...
    for k, t in enumerate(tokens):
        if k != 0 and IN_EN_CHARS.get(tokens[k - 1][-1]) and IN_EN_CHARS.get(t[0]):
            new.append(" ")
        new.append(t)
    return "".join(new)
